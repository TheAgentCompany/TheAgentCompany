...
def remove_func(file_path=UT_FILE, function_name=f"def {UT_FUNC_NAME}("):
    """
    Remove UT function from the specified test file.
    """
    try:
        ...
        if start_idx is not None:
            ...
            print(f"Successfully removed UT function")
            return True

        print(f"Function {UT_FUNC_NAME} not found in file")
        return False
...
def checkpoint3(path=UT_FILE, function_name=UT_FUNC_NAME):
    return is_function_exists(file_path=path, function_name=function_name)

def checkpoint4(cov_file_path=COV_FILE, ut_file_path=UT_FILE, function_name=UT_FUNC_NAME):
...
def checkpoint5(cov_file_path=COV_FILE, ut_file_path=UT_FILE):
    ...
    remove_func(file_path=UT_FILE, function_name=f"def {UT_FUNC_NAME}(")
