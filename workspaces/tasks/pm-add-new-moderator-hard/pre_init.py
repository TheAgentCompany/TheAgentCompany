...
def add_user_role_moderator(channel_name, username):
    if check_user_role(username, 'moderator') == True:
        logger.info("Role already assigned")
        return False
    response_user = rocket.users_info(username = username).json()
    user_id = response_user['user']['_id']
    response_channel = rocket.channels_info(channel=channel_name).json()
    channel_id = response_channel['channel']['_id']
    response = rocket.channels_add_moderator(channel_id, user_id).json()
    if response.get('success'):
        logger.info(f"Successfully assigned {username} as moderator.")
        return True
    else:
        logger.error(f"Failed to assign {username} as moderator.")
        return False

if __name__ == "__main__":
    channel_name_1 = "kudos"
    channel_name_2 = "general"
    username1 = 'John.Doe'
    username2 = 'li_ming'
    username3 = 'huang_jie'
    username4 = 'emma_lewis'
    print(add_user_to_channel(channel_name_1, username1))
    print(add_user_to_channel(channel_name_1, username2))
    print(add_user_to_channel(channel_name_2, username3))
    print(add_user_to_channel(channel_name_2, username4))
    print(add_user_role_moderator(channel_name_1, username1))
    print(add_user_role_moderator(channel_name_1, username1))
    print(add_user_role_moderator(channel_name_2, username2))
    print(add_user_role_moderator(channel_name_2, username2))
