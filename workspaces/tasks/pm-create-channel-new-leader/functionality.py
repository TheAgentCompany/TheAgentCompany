...
def add_user_role_leader(channel_name, username):
    if check_user_role(username, 'leader') == True:
        logger.info("Role already assigned")
        return False
    response_user = rocket.users_info(username = username).json()
    user_id = response_user['user']['_id']
    response_channel = rocket.channels_info(channel=channel_name).json()
    channel_id = response_channel['channel']['_id']
    response = rocket.channels_add_leader(channel_id, user_id).json()
    if response.get('success'):
        logger.info(f"Successfully assigned {username} as leader.")
        return True
    else:
        logger.error(f"Failed to assign {username} as leader.")
        return False

if __name__ == "__main__":
    channel_name = "visualwebagent"
