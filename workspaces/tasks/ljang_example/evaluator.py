...
def check_code_clone():
    ...
    if os.path.exists('/workspace/janusgraph'):
        with open('/workspace/janusgraph/README.MD') as f:
            code_content = f.read()
            if f"JanusGraph is a highly scalable [graph database](https://en.wikipedia.org/wiki/Graph_database)
                ...
                can support thousands of concurrent users, complex traversals, and analytic graph queries." in code_content:
                return True
    return False
